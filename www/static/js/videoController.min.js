...
1>a.media.volume?($(a.controls.mute).children().hide(),$(a.controls.mute).children(".half-volume").show()):($(a.controls.mute).children().hide(),$(a.controls.mute).children(".high-volume").show())});this.$media.on("timeupdate",function(c){a.controls.seekBar&&$(a.controls.seekBar).val(a.getCurrentTime()/a.getDuration());a.controls.timer&&(a.getStringTime(),$(a.controls.timer).text(a.getStringTime()))});this.$media.on("keypress",function(c){if(32===c.which)return a.isPlaying()?a.pause():a.play(),c.preventDefault(),
c.stopPropagation(),!1;39===c.keyCode?(c=a.getCurrentTime(),a.setTime(c+5)):37===c.keyCode&&(c=a.getCurrentTime(),a.setTime(c-5))});this.$media.click(function(c){if(1===c.which)return a.isPlaying()?a.pause():a.play(),c.preventDefault(),!1})};GenericMediaController.prototype.setVolume=function(a){this.media.volume=a};GenericMediaController.prototype.setContainer=function(a){this.container=document.querySelector(a);this.$container=$(a)};
